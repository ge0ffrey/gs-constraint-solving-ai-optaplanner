...
all lessons are re-evaluated to calculate the score.

Instead, create `src/main/java/com/example/solver/TimeTableConstraintProvider.java`
to do incremental score calculation.
It uses OptaPlanner's ConstraintStream API that feels akin to Java 8 Streams and SQL:
...
so your `TimeTableConstraintProvider` can select _from_ these.
...
which can cause an HTTP timeout. To avoid, limit the solving time to 5 seconds
