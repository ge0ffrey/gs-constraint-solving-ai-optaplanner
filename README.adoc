...
If you choose Maven, your `pom.xml` file looks like this:
...
On the other hand, in Gradle, your `build.gradle` file resembles this:
...
and there are no timeslots during lunch or other breaks.
...
Because a timeslot does not change during solving, it is called a _problem fact_.

Notice the `toString()` method keeps the output short
so it is easy to read OptaPlanner's `DEBUG` or `TRACE` log, as shown later.

=== Room

A `Room` is a location where lessons are taught,
for example, `Room A` or `Room B`.
For simplicity's sake, all rooms are without capacity limits and can accommodate all lessons.
...
A room does not change during solving, so it is also a _problem fact_.

=== Lesson

During a `Lesson`, a teacher teaches a subject to a group of students,
...
During solving, OptaPlanner changes two fields of the `Lesson` class,
to assign each lesson to a `timeslot` and `room`.
Because OptaPlanner changes these fields, a `Lesson` is a _planning entity_:
...
Annotate the `Lesson` class with a `@PlanningEntity` annotation,
so OptaPlanner knows that this class changes during solving
because that class has one or more planning variables.

Annotate the `timeslot` field with a `@PlanningVariable` annotation,
...
This use case uses a `HardSoftScore` because it has hard and soft constraints:
...
This example scales an order of magnitude better: __O__(n) instead of __O__(n²).
...
* If all lessons are still unassigned, then it is an _uninitialized_ solution,
...
OptaPlanner knows that this class contains all of the input and output data.
...
* A `lessonList` field for which each `Lesson` instance now has non-null `timeslot` and `room` fields
...
To avoid this, limit the solving time to five seconds the
...
Create the `src/main/java/com/example/TimeTableSpringBootApp.java` class:
...
After about five seconds, the service returns output similar to the following example:
...
Notice that your application assigned all four lessons to one of the two timeslots and one of the two rooms.
...
This test verifies that after solving all lessons are assigned to a timeslot and a room,
as well as verifies that it found a feasible solution (no hard constraints broken).
...
This avoids hard coding a solver time. The unit test can run on arbitrary hardware.
This approach ensures that the test runs long enough to find a feasible solution, even on slow machines.
But it does not run a millisecond longer than it strictly must, even on fast computers.
...
=== Next step: Database and UI integration
...
but it is straightforward to enable multi-tenancy and handle multiple `TimeTable` instances in parallel.
+
