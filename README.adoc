...
to deliver a near-optimal solution in a reasonable amount of time.
...
A room also does not change during solving, so it is also a _problem fact_.
...
* If it breaks hard constraints, then it's an _infeasible_ solution,
...
* A `timeslotList` with all timeslots.
** This is a list of problem facts, because they do not change during planning.
* A `roomList` with all rooms.
** This is a list of problem facts, because they do not change during planning.
...
Create the `src/main/java/com/example/solver/TimeTableController.java`:
...
After about five seconds, it returns output similar to the following example:
...
Notice that it assigned all four lessons to one of the two timeslots and one of the two rooms.
...
as well verifies that it found a feasible solution (no hard constraints broken).
...
but it's straightforward to enable multi-tenancy and handle multiple `TimeTable` instances in parallel.
+
