...
Because a timeslot doesn't change during solving, it's called a _problem fact_.
...
A room also doesn't change during solving, so it's also a _problem fact_.
...
To avoid this, limit the solving time to 5 seconds
...
. Build a pretty web UI on top of these REST methods to visualize the timetable.
